# OEC Learning — operational close

**Branch:** `feat/learning-l1-l5`
**Baseline:** OEC 3.6.0 / `v3.6.0-scientific-ai`
See also `LEARNING-OPERATIONAL.md`.

| Wave | Status |
|------|--------|
| L1–L4 | operational (core) |
| L5 HF | operational with `oec[foundation]` + `learning_smoke` |
| L6 tabular distill | operational with `oec[neural]` |
| L7–L8–L10 | operational-when-installed (external packages) |
| L9 / L11 | operational |
| L12 Worker | operational FineTune sequence + ExecutionResult evaluate |
| L13–L14 | operational (suite + CI isolation/smoke) |

Out of cut: Kronos, POST-OEC, FM text distillation.
