...
| D-CUR-23 | P2 | Sub-debt of D-CUR-22, deliberately not addressed in that fix: (a) the keyword-overlap ranking in `rank_candidate_skills` is a plain-token heuristic, not guaranteed to surface the single best skill first for an ambiguous request; (b) `control`/`dynamics`/`finance`/`uncertainty` have real skills under `skills/` but no specialist at all, so `_infer_domain_from_request` still can't route to them even via the new fallback — a `request` naming those domains falls through to `agent.default`'s generic "could not infer a specialist" `ValueError` | Still open — ranking quality needs real usage data before investing in anything beyond token overlap; orphan domains need either new specialists or a domain-agnostic catalog-wide fallback, a bigger design call than this fix's scope |
