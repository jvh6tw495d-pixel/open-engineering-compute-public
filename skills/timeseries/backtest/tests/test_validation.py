"""Unit tests for timeseries.backtest's own validation.py."""
...
_validator = validation.BacktestValidator()
...
def test_short_series_is_an_error() -> None:
    assert _errors({"series": [1.0], "steps_ahead": 1, "method": "naive"})


def test_invalid_steps_ahead_is_an_error() -> None:
    assert _errors({"series": [1.0, 2.0], "steps_ahead": 0, "method": "naive"})


def test_n_evaluations_eq_len_is_an_error() -> None:
    assert _errors(
        {"series": [1.0, 2.0, 3.0], "steps_ahead": 1, "method": "naive", "n_evaluations": 3}
    )
...
def test_valid_is_ok() -> None:
    assert not _errors({"series": [1.0, 2.0, 3.0], "steps_ahead": 1, "method": "naive"})